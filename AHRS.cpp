...
void AHRS::SetCoeff(float KpAcc,float KiAcc, float KpMag, float KiMag, float accelCutoff)
{
	_KpAcc = KpAcc;
	_KiAcc = KiAcc;
	_KpMag = KpMag;
...
void AHRS::MargAHRSinit(float ax, float ay, float az, float mx, float my, float mz)
{
...
void  AHRS::MargAHRSupdate(float gx, float gy, float gz,
	...
	float accelCutoff, uint8_t magDataUpdate, float dt)
{
	...
	if (MargAHRSinitialized == true)
	{
		...
		if (norm != 0.0f)
		{
			...
			kpAcc = _KpAcc * accConfidence;
			kiAcc = _KiAcc * accConfidence;
		...
		if ((magDataUpdate == true) && (norm != 0.0f))
		{
			...
			if (_KiMag > 0.0f)
			{
				exMagInt += exMag * _KiMag;
				eyMagInt += eyMag * _KiMag;
				ezMagInt += ezMag * _KiMag;
